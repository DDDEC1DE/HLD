...
AMovingPlatform::AMovingPlatform()
{
	...
	//CALL SetupAttachment() on PlatformSprite passing in SetupAttachment()
    PlatformSprite->SetupAttachment(BoxCollision);
...
void AMovingPlatform::Tick(float DeltaTime)
{
 ...
 //   }
    ...
    // Определение текущего местоположения BoxCollision
    const FVector BoxCollisionLocation = BoxCollision->GetComponentLocation();
    // Вычисление дистанции до текущей целевой точки
    float DistanceToPoint = FVector::Dist(BoxCollisionLocation, CurrentGoToLocation);

    // Проверка, находится ли платформа достаточно близко к целевой точке
    if (DistanceToPoint < 100)
    {
        // Переключение целевой точки
        CurrentGoToLocation = (CurrentGoToLocation == Target1Location) ? Target2Location : Target1Location;
    }

    // Перемещение платформы к текущей целевой точке
    FVector TargetLocation = FMath::VInterpConstantTo(BoxCollisionLocation, CurrentGoToLocation, DeltaTime, InterpSpeed);
    BoxCollision->SetWorldLocation(TargetLocation);
