...
{
    static ConstructorHelpers::FClassFinder<UUserWidget> BlockioHud(TEXT("/Game/Blueprints/UI/BP_BlockioHUD"));
...
void ABlockioHUD::BeginPlay()
{
    ...
    BlockioGUI = CreateWidget<UUserWidget>(GetGameInstance(), BlockioGUIClass);
	//CALL AddToViewport() on BlockioGUI
    BlockioGUI->AddToViewport();
}
